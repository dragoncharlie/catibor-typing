...
const punctuation = ['.', ',', ';', ' -', '?', '!']
const words = ['meow', 'meow-meow', 'mrrrya', 'MEOW', 'nya', 'prrrr', 'miaow', 'mrruh', 'prrrup', 'mrow', 'hiss', 'hissssss', 'mew', 'mrrowow', 'mrrrp', 'kikikiki', 'meep', 'purrr', 'miaow', 'ki-ki-ki']
...
const getRandomWord = (capitalized: boolean, last: boolean) => {
  ...
  return [word + punct, ['.', '!', '?'].includes(punct)]
}

export const generateText = () => {
  const length = getRandomInt(50, 25)
  let sentence = new Array(length).fill('')

  let nextCapitalized = true
  sentence.forEach((_, index) => {
    const [word, capitalized] = getRandomWord(nextCapitalized, index === sentence.length - 1)
    // @ts-ignore
